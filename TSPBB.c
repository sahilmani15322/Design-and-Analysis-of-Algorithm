	...
	while(1)
	{
		...
		else
		{
			...
			for(c=1;c<v;c++)
			{
				...
				{
					if(E->levelno<(v-1) || (E->levelno==(v-1) && *(adjm+c*v)!=INT_MAX)) //checking if c is any other node or c is the answer node and there is an edge between c and root
					{
						if(not_found(path,c,v)) //returns 1 if not found
						{
